...
namespace Chess::Core::Cache {
    ...
    */
    ...
    static constexpr std::array<Bitboard, 64> rookPrimaryMasks = Calc::CalculateRaysForRook();
    static constexpr std::array<Bitboard, 64> bishopPrimaryMasks = Calc::CalculateRaysForBishop();

    static constexpr std::array<Bitboard, 64> knightCache = Calc::CalculateForKnight();
    static constexpr std::array<Bitboard, 64> kingCache = Calc::CalculateForKing();
    std::array<std::vector<Bitboard>, 64> rookCache;
    std::array<std::vector<Bitboard>, 64> bishopCache;

    uint16_t Hash(Square square, Bitboard occupancy, Piece piece) {
        if (piece == Rook)
...
void Chess::Core::Cache::InitCache() {
    Bitboard occupancy;
    uint8_t count;
    ...
    for (uint8_t square = 0; square < 64; ++square) {
        ...
        for (uint16_t index = 0; index < variants; ++index) {
            occupancy = FindOccupancy(index, rookPrimaryMasks[square], count);
        ...
        for (uint16_t index = 0; index < variants; ++index) {
            occupancy = FindOccupancy(index, bishopPrimaryMasks[square], count);
...
Chess::Core::Bitboard Chess::Core::Cache::GetCache(Piece piece, Square square, uint16_t index) {
    switch (piece) {
        case Rook: return rookCache[square][index];
        case Bishop: return bishopCache[square][index];
        case Knight: return knightCache[square];
        case King: return kingCache[square];
        default: return 0ULL;
    }
}

Chess::Core::Bitboard Chess::Core::Cache::GetSlidingMask(Piece piece, Square square) {
    switch (piece) {
        case Rook: return rookPrimaryMasks[square];
        case Bishop: return bishopPrimaryMasks[square];
        default: return 0ULL;
    }
}